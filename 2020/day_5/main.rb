...
class Solution
  ...
  def part_one!
    input.map do |b_pass|
      row = find_section(b_pass[0..6], (0..127), ["F", "B"])
      col = find_section(b_pass[7..], (0..7), ["L", "R"])
      (row * 8) + col
    end.max
  end

  def find_section(values, range, indicators)
    row = range
    values.each do |current|
      if row.size == 2
        row = current == indicators[0] ? row.first : row.last
        break
      end

      mid_point = (row.size / 2) + row.first
      if current == indicators[0]
        row = (row.first..mid_point-1)
      else
        row = (mid_point..row.end)
      end
    end
    row
  end
  ...
  def part_two!
    ids = input.map do |b_pass|
      row = find_section(b_pass[0..6], (0..127), ["F", "B"])
      col = find_section(b_pass[7..], (0..7), ["L", "R"])
      (row * 8) + col
    end.sort

    ids.slice_when { |b, a| a != (b + 1) }.to_a.first.last + 1
  end
end
