...
# incident. While you consider checking the in-flight menu for one of those drinks
# that come with a little umbrella, you are interrupted by the kid sitting next to
# you.
#
...
class Solution
  ...
  def part_one!
    run_simulation(input)[0]
  end

  def part_two!
    current_input, last_change = flip_value(input, -1)
    loop do
      acc, success = run_simulation(current_input)
      return acc if success

      current_input, last_change = flip_value(input, last_change)
    end
  end
  ...
  def run_simulation(commands)
    ...
    loop do
      ...
      break if prevs.include?(index)

      prevs << index
      cmd, n = commands[index]
      case cmd
      when "acc"
        acc += n
        index += 1
      ...
      index = index % commands.count
    end

    return acc, success
  end

  def flip_value(commands, i)
    x = i+1
    new_input = commands.clone
    while true
      cmd, n = new_input[x]
      if ["nop", "jmp"].include? cmd
        new_input[x] = cmd == "nop" ? ["jmp", n] : ["nop", n]
        return [new_input, x]
      else
        x += 1
      end
