...
class Game
  ...
  def self.parse(string)
    game_id_string, rounds_string = string.split(": ")
    game_id = game_id_string.scan(/\d+/).first.to_i
    rounds = rounds_string.split("; ").map { Round.parse(_1) }
    new(
      id: game_id,
      rounds: rounds,
    )
  ...
  def is_possible_with?(max_counts)
    @rounds.all? do |round|
      round.is_possible_with?(max_counts)
    end
  end

  def fewest_counts_possible
    Hash.new(0).tap do |res|
      @rounds.each do |round|
        round.colour_counts.each do |colour, count|
          if res[colour] < count
            res[colour] = count
          end
        end
  ...
  class Round
    attr_reader :colour_counts

    def self.parse(string)
      counts = string.split(", ").map do |count|
    ...
    def is_possible_with?(max_counts)
      colour_counts.all? do |colour, count|
...
def part_one(input)
  ...
  input
    .select { _1.is_possible_with?(max_counts) }
    .map { _1.id }
    .sum
end
...
def part_two(input)
  input
    .map { _1.fewest_counts_possible.reduce(&:*) }
    .sum
end
