...
runner = Runner.new
runner.add_test(:part_one, expected: 114, skip: false, input: <<~MSG)
0 3 6 9 12 15
1 3 6 10 15 21
10 13 16 21 30 45
MSG

runner.add_test(:part_two, expected: 2, skip: false, input: <<~MSG)
0 3 6 9 12 15
1 3 6 10 15 21
10 13 16 21 30 45
MSG
...
module Solution
  ...
  def part_one(raw_input)
    ...
    readings.each do |reading|
      ...
      while true
        ...
        i = 1
        while i < sequences[current_sequence].count
          sequence << sequences[current_sequence][i] - sequences[current_sequence][i-1]
          i += 1
        end
      ...
      while i < sequences.count
        new_num = sequences[-(i+1)][-1] + sequences[-i][-1]
        sequences[-(i+1)] << new_num
        i+=1
      end
  ...
  def part_two(raw_input)
    ...
    readings.each do |reading|
      ...
      i = 1
      while i < sequences.count
        new_num = sequences[-(i+1)][0] - sequences[-i][0]
        sequences[-(i+1)].unshift(new_num)
        i+=1
      end
