...
runner = Runner.new
runner.add_test(:part_one, expected: 288, skip: false, input: <<~MSG)
Time:      7  15   30
Distance:  9  40  200
MSG

runner.add_test(:part_two, expected: 71503, skip: false, input: <<~MSG)
Time:      7  15   30
Distance:  9  40  200
MSG
...
module Solution
  ...
  def part_one(raw_input)
    times, distances = parse(raw_input)
    different_win_holds = []

    i = 0
    while i < times.count
      ways_to_win = 0
      allowed_time = times[i]
      best_distance = distances[i]

      allowed_time.times do |velocity|
        time_left = allowed_time - velocity
        distance_travelled = velocity * time_left
        ways_to_win += 1 if distance_travelled > best_distance
      end
      different_win_holds << ways_to_win
      i += 1
    end

    different_win_holds.reduce(&:*)
  end
  ...
  def part_two(raw_input)
    ...
    time.times do |velocity|
      time_left = time - velocity
      distance_travelled = velocity * time_left
      ways_to_win += 1 if distance_travelled > distance
    end
  ...
  def parse(input)
    input.split("\n")
      .map { _1.split(/:\s+/)[1].split(/\s+/).map(&:to_i) }
  end
end
