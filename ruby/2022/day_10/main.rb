...
class Solution
  ...
  def part_one!
    ...
    while true
      cycle = 19 + (i * 40)
      break if cycle > vals.length - 1

      interesting_vals << vals[cycle] * (cycle + 1)

      i += 1
    end
    interesting_vals.sum
  end
  ...
  def part_two!
    ...
    vals.map! { [_1 - 1, _1, _1 + 1] }
    buffer = ""
    vals.each_with_index do |pixels, i|
      pixel = i % 40
      if pixels.include? pixel
        buffer << "#"
      else
        buffer << "."
      end
