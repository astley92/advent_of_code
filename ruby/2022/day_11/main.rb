...
# turn, it inspects and throws all of the items it is holding one at a time and in
# the order listed. Monkey 0 goes first, then monkey 1, and so on until each
...
def self.parse_input(input)
  input.split("\n\n").map { _1.split("\n").map(&:strip) }.map do |attrs|
    transform = attrs[2].split(" new = old ")[1].split(" ")
    {
      items: attrs[1].split(": ")[1].split(", ").map(&:to_i),
      transform: transform[-1] == "old" ? ["**", 2] : [transform[0], transform[1].to_i],
      divisor: attrs[3].split(" divisible by ")[1].to_i,
      true_case: attrs[4].split(" ")[-1].to_i,
      false_case: attrs[5].split(" ")[-1].to_i,
      inspected_count: 0,
    }
  end
end
  ...
  20.times do
    input.each do |monkey|
      while monkey[:items].any?
        ...
        item = item.send(*monkey[:transform])
        item = (item / 3).round
        monkey_index = item % monkey[:divisor] == 0 ? monkey[:true_case] : monkey[:false_case]
        input[monkey_index][:items] << item
      end
    end
  end

  input.map { _1[:inspected_count] }.sort[-2..].reduce(:*)
end

def part_two(input)
  mod = input.map { _1[:divisor] }.reduce(:*)
