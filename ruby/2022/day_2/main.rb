...
class Solution
  ...
  def part_one!
    score = 0
    input.map do |opp, me|
      [CHAR_TO_SHAPE_MAP[opp], CHAR_TO_SHAPE_MAP[me]]
    end.each do |opp, me|
      case result_of(opp, me)
      when :win
        score += 6
      when :draw
        score += 3
  ...
  def part_two!
    score = 0
    input.map do |opp, res|
      [CHAR_TO_SHAPE_MAP[opp], CHAR_TO_RESULT_MAP[res]]
    end.each do |opp, res|
      me = shape_for_result(opp, res)
      case result_of(opp, me)
      when :win
        score += 6
      when :draw
  ...
  def result_of(opp, me)
    return :draw if opp == me
    my_index = ORDER.index(me)
    win_index = (my_index - 1) % 3
    ORDER.index(opp) == win_index ? :win : :loss
  end

  def shape_for_result(opp, result)
    return opp if result == :draw

    opp_index = ORDER.index(opp)
    if result == :win
      ORDER[(opp_index + 1) % 3]
    else
      ORDER[(opp_index - 1) % 3]
