...
# A job like aaaa + bbbb means the monkey waits for monkeys aaaa and bbbb to yell
# each of their numbers; the monkey then yells the sum of those two numbers.
...
def self.parse_input(input)
  ...
  input.split("\n").map do |monkey_job_string|
    monkey, job_string = monkey_job_string.split(": ")
    job_args = job_string.split(" ")
    if job_args.count == 1
      job_args = job_args.first.to_i
    end
    [monkey, job_args]
  end.to_h
end
...
def part_one(input)
  while !input["root"].is_a?(Integer)
    input = reduce_to_ints(input)
  end
  input["root"]
end

def reduce_to_ints(monkeys)
  result = monkeys.dup
  monkeys.each do |name, args|
    next if args.is_a?(Integer)

    one, operation, two = args
    if result[one].is_a?(Integer) && result[two].is_a?(Integer)
      result[name] = result[one].send(operation, result[two])
    end
  end
  result
end
...
def part_two(input)
  input["root"][1] = "=="
  Parallel.each((0..100_000_000), in_processes: 8) do |humn_num|
    if check_for_num(humn_num, input.dup)
      puts humn_num
...
def check_for_num(humn_num, current_input)
  puts "Checking #{humn_num}"
  current_input["humn"] = humn_num

  while !([true, false].include? current_input["root"])
    current_input = reduce_to_ints(current_input)
  end

  if current_input["root"] == true
    return humn_num
  end
end
