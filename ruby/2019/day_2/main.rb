...
runner = Runner.new
runner.add_test(:part_one, expected: 30, skip: true, input: <<~MSG)
1,1,1,4,99,5,6,0,99
MSG

runner.add_test(:part_two, expected: nil, skip: true, input: <<~MSG)
MSG
...
module Solution
  ...
  def part_one(raw_input)
    ...
    while true
      case parsed_input[i]
      when 1
        left, right, store = parsed_input[i+1..i+3]
        parsed_input[store] = parsed_input[left] + parsed_input[right]
      when 2
        left, right, store = parsed_input[i+1..i+3]
        parsed_input[store] = parsed_input[left] * parsed_input[right]
      when 99
        ...
        raise NotImplementedError, "Unkown Op Code: #{parsed_input[i].inspect}"
      end
  ...
  def part_two(raw_input)
    ...
    50.times do |a|
      50.times do |b|
        parsed_input = parse(raw_input).dup
        parsed_input[1] = a
        parsed_input[2] = b
